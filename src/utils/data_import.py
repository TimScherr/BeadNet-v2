...
class DataImportWorker(QObject):
    ...
    def import_data(self, img_ids, keep_normalization, keep_resolution, scale_factor, crop_size, trainset_id,
                    train_path, omero_username, omero_password,  omero_host, omero_port, group_id, p_train, p_val,
                    p_test):
        ...
        :param keep_resolution: Keep initial resolution of the imported images
        :type keep_resolution: bool
        ...
        for i, img_id in enumerate(img_ids):
            ...
            if keep_resolution:
                ...
                img = rescale(img, scale_factor, preserve_range=True, order=2).astype(img.dtype)
                mask = rescale(measure.label(mask, background=0),
                               scale_factor, preserve_range=True, order=0).astype(mask.dtype)

            # Convert mask to centroids
            mask = mask2centroid(mask, img)
            ...
            for img, mask, x_start, y_start in img_list:
                ...
                for j in range(len(props)):
                    ...
                    mask_point_roi.theZ = rint(0)
                    mask_point_roi.theT = rint(0)
                    mask_point_roi.strokeColor = rint(int.from_bytes([255, 255, 0, 255], byteorder='big', signed=True))
                ...
                update_service.saveAndReturnObject(create_roi(omero_img, mask_point_roi_list))
...
def mask2centroid(mask: npt.NDArray[Union[bool, np.ubyte, np.ushort, np.uintc, np.uint]],
    ...
    mean_particle_intensity = np.mean(intensity_image, where=mask > 0)
    mean_background_intensity = np.mean(intensity_image, where=mask == 0)
    if mean_background_intensity > mean_particle_intensity:  # particles are dark
        intensity_image = intensity_image.max() - intensity_image  # invert for weighted centroid calculation

    # Get centroids
    centroid_list = []
    props = measure.regionprops(mask, intensity_image=intensity_image)
    for j in range(len(props)):
        # Use weighted centroids
        centroid_list.append(props[j].centroid_weighted)
    # Convert mask to centroids
    mask = np.zeros_like(mask)
    for j, centroid in enumerate(centroid_list):
        mask[round(centroid[0]), round(centroid[1])] = j + 1
