...
from src.training.losses import bce_dice
...
from src.utils.unets import build_unet
...
class CreateLabelsWorker(QObject):
    ...
    def create_labels(self, path, label_type):
        ...
        for i, mask_id in enumerate(mask_ids):
            ...
            # Calculate labels
            label = get_label(mask=mask, label_type=label_type)

            # Save labels
            fname = mask_id.name.split('mask_')[-1]
            tiff.imwrite(str(mask_id.parent / f'{label_type}_{fname}'), label)
...
class TrainWorker(QObject):
    ...
    def start_training(self, path_data, path_models, label_type, iterations, optimizer, batch_size, device, num_gpus):
        ...
        for i in range(iterations):
            ...
            filters = [64, 512]  # Number of kernels (start and maximum)

            try_training = True
            while try_training:
                try:
                    # Get CNN settings
                    train_configs = {'architecture': ('U-Net',
                                                      "conv",
                                                      act_fun,
                                                      'bn',
                                                      filters),
